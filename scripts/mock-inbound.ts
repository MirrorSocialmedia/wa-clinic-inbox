...
function buildHistory(clinicPhoneId: string, bizNumber: string, o: { from: string; count: number; name?: string }): unknown {
  const now = Math.floor(Date.now() / 1000);
  const msgs: WaMsg[] = [];
  // 30 日內嘅舊 chat：病人/店員各半，亂序（測試容忍亂序）
...
async function main(): Promise<void> {
  if (!cmd || !["message", "echo", "status", "history", "unknown"].includes(cmd)) {
    ...
    console.error("  history: --clinic <code> --from <waId> [--count N] [--name]");
