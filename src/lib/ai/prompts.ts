...
export function buildSystemPrompt(lexiconBlock = ""): string {
  return [
    ...
    "",
    "sessionTrigger（病人係咪想開銷售諮詢 — 2 選 1 或 null；唔影響 intent 分類）：",
    "- ORTHODONTIC_CONSULT：病人想知多啲 / 想預約 矯正（箍牙 / 矯齒 / 隱形牙套 / Invisalign 類）",
    "- IMPLANT_CONSULT：病人想知多啲 / 想預約 植牙（缺牙植入 / 種牙類）",
    "- 其他一切（預約檢查 / 痛症 / 查詢 / 多謝…）一律 null；唔肯定就 null，唔好估。",
    "",
...
export const CLASSIFY_DRAFT_JSON_SCHEMA = {
  ...
  properties: {
    ...
    // ★ consult v2.1 C1（§2.2 M-2）：null = 非 consult；2 個 workflow 值（floor 由 code 兜底，LLM 只係輔助訊號）
    sessionTrigger: { type: ["string", "null"], enum: ["ORTHODONTIC_CONSULT", "IMPLANT_CONSULT", null] },
